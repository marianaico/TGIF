...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    stats.glance.forEach(p => {
        document.getElementById("loyalty-body").innerHTML +=
            `<tr><td>${p.party}</td><td>${p.avgVotes}%</td></tr>`;
    });

    render("least-loyal", stats.leastLoyal, "votes_with_party_pct");
    render("most-loyal", stats.mostLoyal, "votes_with_party_pct");
});
